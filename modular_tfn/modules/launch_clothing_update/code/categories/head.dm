//Loadout Stuff
//Hats
/datum/loadout_item/head/hat/bucket
	name = "Bucket Hat"
	item_path = /obj/item/clothing/head/vampire/hat/bucket

/datum/loadout_item/head/hat/boater
	name = "Boater Hat"
	item_path = /obj/item/clothing/head/vampire/hat/boater

/datum/loadout_item/head/hat/beaver
	name = "Beaver Hat"
	item_path = /obj/item/clothing/head/vampire/hat/beaver

/datum/loadout_item/head/hat/wool
	name = "Wool Hat"
	item_path = /obj/item/clothing/head/vampire/hat/wool

/datum/loadout_item/head/hat/wool/blue
	name = "Blue and White Wool Hat"
	item_path = /obj/item/clothing/head/vampire/hat/wool/blue

/datum/loadout_item/head/picture
	name = "Picture Hat"
	item_path = /obj/item/clothing/head/vampire/picture

/datum/loadout_item/head/picture/red
	name = "Red Picture Hat"
	item_path = /obj/item/clothing/head/vampire/picture/red

//Costume Headwear
/datum/loadout_item/head/costume
	group = "Costume Headwear"
	abstract_type = /datum/loadout_item/head/costume

/datum/loadout_item/head/costume/headband_frilly
	name = "Frilly Headband"
	item_path = /obj/item/clothing/head/vampire/headband_frilly

/datum/loadout_item/head/costume/headdress_black
	name = "Black Headdress"
	item_path = /obj/item/clothing/head/vampire/headdress_black
